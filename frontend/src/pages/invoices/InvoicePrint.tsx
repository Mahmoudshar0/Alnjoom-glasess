...
export default function InvoicePrint() {
  ...
  return (
    <div className={isRtl ? 'font-[Tahoma,Segoe_UI,sans-serif]' : ''} dir={isRtl ? 'rtl' : 'ltr'} lang={lang}>
      {/* Screen-only toolbar */}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-8 max-w-3xl mx-auto print:border-none print:rounded-none print:shadow-none print:p-0 print:max-w-none">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div className={isRtl ? 'text-right' : ''}>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t.billTo}</p>
            <p className="text-base font-semibold text-slate-900">{invoice.customer?.name ?? '—'}</p>
            {invoice.customer?.phone && <p className="text-sm text-slate-600 mt-0.5">{invoice.customer.phone}</p>}
          </div>
          <div className={detailsAlign}>
            <div className="space-y-1.5 text-sm">
              <div className={`flex gap-6  ${isRtl ? 'justify-start' : 'justify-end'}`}>
                <span className="text-slate-500">{t.invoiceDate}</span>
                <span className={`font-medium text-slate-900 w-32 ${detailsAlign}`}>
                  {formatPrintDate(invoice.createdAt, lang)}
                </span>
              </div>
              <div className={`flex gap-6  ${isRtl ? 'justify-start' : 'justify-end'}`}>
                <span className="text-slate-500">{t.orders}</span>
                <span className={`font-medium text-slate-900 w-32 ${detailsAlign}`}>{invoice.orders?.length ?? 0}</span>
              </div>
        ...
        <div className="mb-6">
          {invoice.orders && invoice.orders.length > 0 ? (
              ...
              <div key={order.id} className={oi > 0 ? 'mt-6' : ''}>
                <div className="flex items-center justify-between mb-2 pb-1 border-b border-slate-200">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">
                    {t.order} {oi + 1} — {formatPrintDate(order.createdAt, lang)}
                  </p>
                </div>
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className={`py-2 text-xs font-bold text-slate-600 uppercase tracking-wide ${isRtl ? 'text-right' : 'text-left'}`}>
                        {t.description}
                      </th>
                      <th className="text-center py-2 text-xs font-bold text-slate-600 uppercase tracking-wide w-16">{t.qty}</th>
                      <th className={`py-2 text-xs font-bold text-slate-600 uppercase tracking-wide w-28 ${isRtl ? 'text-left' : 'text-right'}`}>
                        {t.unitPrice}
                      </th>
                      <th className={`py-2 text-xs font-bold text-slate-600 uppercase tracking-wide w-28 ${isRtl ? 'text-left' : 'text-right'}`}>
                        {t.amount}
                  ...
                  <tbody>
                    {order.items.map((item, i) => {
                      const name = item.inventoryItem
                        ...
                        : item.customItemName || '—';
                      return (
                        <tr key={item.id} className={`border-b border-slate-50 ${i % 2 === 0 ? '' : 'bg-slate-50/50'}`}>
                          <td className={`py-2.5 text-slate-900 font-medium ${isRtl ? 'text-right' : 'text-left'}`}>
                            {name}
                          ...
                          <td className={`py-2.5 text-slate-700 ${isRtl ? 'text-left' : 'text-right'}`}>{formatKWD(item.price)}</td>
                          <td className={`py-2.5 font-medium text-slate-900 ${isRtl ? 'text-left' : 'text-right'}`}>
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={3} className={`py-2 text-xs text-slate-500 pr-2 ${isRtl ? 'text-left pl-2 pr-0' : 'text-right'}`}>
                        {t.orderSubtotal}
                      </td>
                      <td className={`py-2 font-semibold text-slate-900 ${isRtl ? 'text-left' : 'text-right'}`}>
                        {formatKWD(order.items.reduce((s, i) => s + i.price * i.quantity, 0))}
                      </td>
                    </tr>
                  </tfoot>
                </table>
            ...
            <p className="text-sm text-slate-400 italic text-center py-4">{t.noLineItems}</p>
          )}
        </div>
        ...
        <div className={`flex mb-6 ${isRtl ? 'justify-start' : 'justify-end'}`}>
          <div className="w-64 space-y-2 text-sm">
            <div className={`flex justify-between border-b border-slate-200 pb-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
            ...
            {invoice.paidAmount > 0 && (
              <div className={`flex justify-between text-emerald-700 ${isRtl ? 'flex-row-reverse' : ''}`}>
        ...
        {invoice.payments && invoice.payments.length > 0 && (
          <div className="mb-6 bg-slate-50 rounded-lg p-4">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">{t.paymentHistory}</p>
            <div className="space-y-1.5">
              {invoice.payments.map(p => (
                <div key={p.id} className={`flex justify-between text-sm ${isRtl ? 'flex-row-reverse' : ''}`}>
                  <span className="text-slate-600">
                    {formatPrintDate(p.date, lang)} · {paymentMethodLabel(p.method, lang)}
                  </span>
        ...
        {invoice.notes && (
          <div className="mb-6 text-sm">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{t.notes}</p>
            <p className="text-slate-700">{invoice.notes}</p>
          </div>
        )}

        {/* Footer */}
        <div className={`mt-8 pt-6 border-t border-slate-200 flex justify-between text-xs text-slate-400 ${isRtl ? 'flex-row-reverse' : ''}`}>
          <p>{t.footerShop}</p>
          <p>
            {t.printed}{' '}
