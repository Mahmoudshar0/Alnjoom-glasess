  ...
  return (
    <div>
      ...
      <div className="no-print flex items-center justify-between mb-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 cursor-pointer transition-colors">
          <ArrowLeft size={16} /> Back
        </button>
        <Button leftIcon={<Printer size={15} />} onClick={() => window.print()}>Print Invoice</Button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-8 max-w-3xl mx-auto print:border-none print:rounded-none print:shadow-none print:p-0 print:max-w-none">
          ...
          <div className="text-right">
            <div className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-end gap-6">
                ...
                <span className="font-medium text-slate-900 w-32 text-right">{invoice.orders?.length ?? 0}</span>
              </div>
        ...
        <div className="mb-6">
          {invoice.orders && invoice.orders.length > 0 ? (
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="text-left py-2 text-xs font-bold text-slate-600 uppercase tracking-wide">Description</th>
                      <th className="text-center py-2 text-xs font-bold text-slate-600 uppercase tracking-wide w-16">Qty</th>
                      <th className="text-right py-2 text-xs font-bold text-slate-600 uppercase tracking-wide w-28">Unit Price</th>
                      <th className="text-right py-2 text-xs font-bold text-slate-600 uppercase tracking-wide w-28">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {order.items.map((item, i) => {
                      ...
                      return (
                        <tr key={item.id} className={`border-b border-slate-50 ${i % 2 === 0 ? '' : 'bg-slate-50/50'}`}>
                          ...
                          <td className="py-2.5 text-right text-slate-700">{formatKWD(item.price)}</td>
                          <td className="py-2.5 text-right font-medium text-slate-900">{formatKWD(item.price * item.quantity)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot>
                    <tr>
                      <td colSpan={3} className="py-2 text-right text-xs text-slate-500 pr-2">Order subtotal</td>
            ...
            <p className="text-sm text-slate-400 italic text-center py-4">No line items</p>
          )}
        </div>
